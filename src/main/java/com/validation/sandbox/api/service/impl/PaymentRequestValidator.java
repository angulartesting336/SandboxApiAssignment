...
public class PaymentRequestValidator {
	...
	public void validatePaymentRequest(PaymentInitiationRequest paymentInitiationRequest, String signatureCertificate,String signature,String xRequestId) {
	...
	/*	if (!formatValidator(initialStream).isEmpty()) {
			...
			paymentRejectedResponse.setReason("Got rejected due to IBAN or Amount validation failed");
			paymentRejectedResponse.setReasonCode("INVALID_REQUEST");
			paymentRejectedResponse.setStatus("Rejected");

			throw new InvalidRequestException(paymentRejectedResponse, signatureCertificate,signature,xRequestId);
