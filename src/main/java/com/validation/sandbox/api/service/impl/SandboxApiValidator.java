...
public class SandboxApiValidator implements SandboxValidationService {
	...
	public PaymentAcceptedResponse validatePaymentRequest(PaymentInitiationRequest paymentInitiationRequest,
		...
		certificateNameValidator.validateCertificateName(signature, signatureCertificate,
				xRequestId);
		
		signatureValidator.validateSignature(paymentInitiationRequest, signature, signatureCertificate,
				xRequestId);
		
		paymentRequestValidator.validatePaymentRequest(paymentInitiationRequest, signature, signatureCertificate,
				xRequestId);
		
		limitExceededValidator.checkLimitExceeded(paymentInitiationRequest.getAmount(),
				paymentInitiationRequest.getDebtorIBAN(), signature, signatureCertificate,
				xRequestId);
